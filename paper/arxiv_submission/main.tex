...
runs, per-run range $[-0.60,+0.05]$), whereas error measured after the
...
$[-0.60,+0.05]$ and changes sign between the two pairs. The ordering therefore
holds run by run, but five variants are
...
and six runs; the per-run ranges are $[-0.60,+0.05]$, $[-1.00,-0.80]$ and
