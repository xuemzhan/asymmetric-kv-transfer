...
rank these variants (rank correlation $-0.10$), whereas error measured after the
student's attention output and after its $o$-projection does ($-1.00$ and
$-0.80$). A rank-8 correction of the
...
synthetic domain repairs is bound to that task distribution. All experiments are
...
\item \textbf{A corrected evaluation and a content-control suite (negative
  ...
  first-token top-1 agreement $0.929$--$0.982$, median $KL$ $0.001$--$0.005$
  nats). A cache round-trip and an explicit position offset each reproduce the
  reference bit for bit while the same residual persists under eager attention,
  so the residual is a low-precision attention-kernel path difference rather
  than a defective evaluator. Under it, no pair's three-seed mean
  ...
  V-only, and joint $0.000$), where the same validator holds (top-1 agreement
  $0.933$; identical Self exact match under both paths). Our own pre-registered
  gate for this project used
...
\item \textbf{A consumer-side diagnosis.} We remove the state-side explanations.
  ...
  against the state they produce separates the two spaces: raw representation
  error does not rank them (Spearman $-0.10$ against EM, pooled over six runs),
  while attention-output error and $o$-projection-space error do ($-1.00$ and
  $-0.80$), with the shuffled-target control the visible exception.
...
\item \textbf{A consumption-side fix.} A rank-8 correction of the student's
  ...
  unadapted $0.899$. With the \emph{final} $20$-epoch adapter frozen and the
  injected content destroyed, the arms separate cleanly on $1.7$B$\to0.6$B
  (correct teacher KV $0.905\pm0.083$ across three seeds, against at most
  $0.179$ for wrong-document, random, and zero caches) but not on
  $8$B$\to0.6$B (mean margin $+0.071$, with a document-clustered interval that
  includes zero in one seed), so the causal reading is established on the
  equal-depth pair and remains unresolved on the flagship pair. The same adapter
  trained on the student's own states stays much lower
  ...
  same mapper and the same adapter leave K-only and joint EM at $0.000$. Nor
  does it reappear within the second domain: retraining the mapper and the
  adapter on SQuAD calibration documents still answers $0.000$ of the held-out
  questions on every transfer arm in all three document splits, even though the
  same recipe recovers $0.94$ V-only EM on the synthetic distribution. We state
  these splits rather than claim content-specific adaptation alone.
\end{enumerate}
...
validator holds on the second domain (Section~5.3).
...
$-0.10$ for the raw error. One variant is a genuine exception and we report it as
...
EM is corrected V-only EM. Rank correlation with EM over the five variants:
$-0.10$ (raw), $-1.00$ (attention output), $-0.80$ ($o$-projection space).}
...
a claim that no mapper could ever work on SQuAD.
