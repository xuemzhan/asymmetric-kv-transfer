...
against $0.899\pm0.010$ for the student's own cache; the same evaluator therefore
does detect usable context. SQuAD replicates
the negative result (Self $0.367$; K-only, V-only, and Joint $0.000$; K-only
$\dll$ $+5.24$).

The failure lies in how the receiving model reads the state, not in the state's
addressing geometry. Offsetting or randomly permuting the layer map does not
reduce transfer, so layer alignment alone does not explain the failure;
...
\item \textbf{A corrected evaluation and a content-control suite (negative
  ...
  cache; the evaluator therefore does detect usable context when it is present.
...
\item \textbf{A consumer-side diagnosis.} We remove the state-side explanations.
  Layer alignment alone does not explain the failure: on the equal-depth pair,
...
is not enough, by a small adaptation of the consumer itself. Our results are
strong at the first level: the value mapper fit in consumption space recovers most
of V-only transfer without changing a single consumer weight, and only the
residual gap needs the second. The mismatch is diagnosable (routing divergence)
...
(unmapped) V-only injection. A simultaneous permutation of the student's own keys
and values is not a content control---attention is invariant to a shared
reordering of $K$ and $V$---so we do not include one; every control above destroys
content. For the adapter we additionally train on student KV and on
...
Every one of these arms yields corrected EM at or near $0$. The same evaluator
scores $0.899 \pm 0.010$ on the student's own cache, so it is not insensitive to
context: it reads usable context and returns nothing for teacher-injected states.
We also note that reordering the token axis of $K$ and $V$ by the \emph{same}
permutation leaves the attention output unchanged, so such a shuffle is not a
content-destroying control; a control must break the key--value correspondence to
be informative, and we do not report a same-permutation arm. The same pattern
holds at seed 0 for the
...
\subsection{Layer alignment alone does not explain the failure}
...
$0.036$ and $0.411$ respectively). This ablation is preliminary (seed 0, two of
the three attention projections) and we report it as suggestive evidence that
locating the correction at the attention output is the stronger and more
content-specific choice. Read together with the mapper results above, it is
consistent with a bottleneck at the attention-output consumption stage rather
than at the state projections.
...
adapting the consumer itself. Changing the state's layer alignment does neither.
