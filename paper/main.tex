...
confirm both defects in the published evaluation code, and we validate the
corrected evaluator against a full-prefill reference on the whole Self test set
(normalized-EM agreement $0.929$--$0.982$; first-token logit error at most
$1.35$). Under that evaluator, no pair averages more than $0.149$ EM for keys or
...
the state correctly. Under mapped teacher keys the student's top-1 attention
agreement with its own routing is $0.59$--$0.70$. Fitting the value mapper where the student reads
...
\item \textbf{A corrected evaluation and a content-control suite (negative
  ...
  first-token logit error at most $1.35$). Under it, no pair's three-seed mean
...
\item \textbf{A consumer-side diagnosis.} We remove the state-side explanations.
  ...
  floor effect of the mapper. What does change is the consumer's
...
alone recovers about a quarter of Self and the adapter is required. The mismatch
is diagnosable (routing divergence)
and repairable, and evaluations of cross-model transfer should include content
controls.
...
generated token's logits is $1.34$, $0.88$, and $0.97$. Greedy tokenization
...
student: the corrected path is end-to-end equivalent to full prefill on the
quantity we report.
...
to within $1.34$ in absolute value, and Self exact match under the two paths
...
teacher arm exceeds $0.15$ when averaged over seeds. At seed 0---the only seed for
which the source archives document-clustered intervals for EM---the best arms of
that pair carry CI95 $[0.375, 0.482]$ (V-only), $[0.071, 0.214]$ (K-only), and
$[0.000, 0.054]$ (Joint), all far below Self.
...
cell is a $0.000$ arm in all three seeds. Document-clustered CI95 for EM is
archived for seed 0 only and is quoted in the text for the largest arms.}
\label{tab:main}
...
are unstable; corrected V-only EM is $0.000$ for every map and seed, and K-only
never exceeds $0.107$. Improving the layer map does not create transfer on this
...
($0.792$, $0.357$, $0.470$). The adapter also alters the student's own
readout---by $+0.041$ on $1.7$B$\to0.6$B and $+0.041$ on $8$B$\to0.6$B relative
to the unadapted $0.899$---so an unadapted Self baseline would have understated
...
arm's likelihood \emph{worse} ($\dll$ $+4.62$ against $+1.05$ under the default
protocol) while EM stays at $0.000$, so on this set it moves the state further
from usable rather than closer. The adapter is not inert: it raises the student's
own SQuAD score from $0.367$ to $0.600$ without seeing SQuAD data, so it does
transfer some generic benefit to the student's reading. It simply does not make
...
Third, the failure is a compatibility failure, and it is worth separating two
levels. \emph{State-space} compatibility---how close the mapped representation is
to the student's own---is not sufficient: the affine mapper minimises
representation error and still leaves the state unusable. \emph{Consumption-space}
compatibility is what tracks task performance, and it can be restored either by
changing \emph{how the consumer reads} the state (a mapper fit in consumption
space, which leaves every consumer weight untouched) or, for the residual gap, by
adapting the consumer itself.

The role of layer alignment depends on which level is in play, and our own earlier
...
\item \textbf{Corpus effects.} Every document shares a template, and each
  ...
  four-arm EM table reports std over seeds and no interval: document-clustered EM
  intervals exist only in the seed-0 reports, where the clustering fix landed after
  seeds 1 and 2 had been written, and we quote those seed-0 intervals in the text
  rather than mix them into a three-seed table. Its std is therefore likely too
  narrow given the document clustering, and we do not use it to separate the
  $8$B$\to4$B arms from zero.
...
\item \textbf{Evaluator provenance.} Our corrected numbers come from our own
  ...
  the first-token logits (error $\le1.34$), not token-for-token. Legacy numbers
...
\item \textbf{Unexplained baseline spread.} Self EM differs by student
  ...
  failure counts are $5$, $32$, and $11$ of $56$, so the $1.7$B dip is not a
  formatting artifact we can see. We still cannot explain it. It does not affect
  within-pair arm comparisons, which is what our claims rest on, but it means
  absolute EM levels are not comparable across pairs.
\item \textbf{Partial seed coverage.} Two results are seed 0 only: the
  repaired-regime layer scrambling (Section~6.1) and the projection ablation of
  Table~\ref{tab:ablation}. The $1.7$B$\to0.6$B control suite, the identity
  V-only control, the random-KV control, and both causality runs are seed 0 only
  as well, whereas the $8$B controls and all four-arm scans have three seeds.
  Where a comparison matters for a headline claim we used a three-seed run.
...
mappers fit in the consumer's space recover the value arm on the equal-depth pair
without touching a single consumer weight, and a small rank-8 adaptation of the
student's output projections recovers most of the K, V, and joint arms there and
part of them on the flagship pair, though not on a second domain. Layer alignment
returns to the picture once the consumer reads the state, which the ordinary
affine mapper hides. Evaluation of cross-model cache reuse should measure task
performance with content controls, and reuse systems should expect to adapt the
consumer.
