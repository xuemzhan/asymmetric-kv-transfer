"""V3 Phase 0：生死门 G0 v2 —— 强映射器下 V 是否仍不可传输？（v2 数据扩展版）

对应 V3 方案 §3 Phase 0 / §4 Gate G0。与 phase0_g0.py 相同协议，仅更换
数据源（train_v2_seed{seed}.json / test_v2_seed{seed}.json，n_calib=70, n_eval=56），
统计工具换成 stats_utils.bootstrap_ci95，并新增 Affine 三臂 vs Self 的 Wilcoxon 检验。
...
def run_g0(seed: int, n_calib: int, n_eval: int, output: str, eval_override=None, run_probe=False) -> dict:
    ...
    def agg(key):
        ...
        mean, ci_low, ci_high = bootstrap_ci95(xs, n_boot=10000, seed=seed)
        return {
            "mean": mean,
            "ci95_low": ci_low,
            "ci95_high": ci_high,
    ...
    # ---------- Wilcoxon: Affine 三臂 vs Self（配对、双尾） ----------
    self_ll = np.array([r["Self"] for r in rows], dtype=float)
    wilcoxon = {}
    for arm in ["K-only", "V-only", "Joint"]:
        arm_ll = np.array([r[f"Affine_{arm}"] for r in rows], dtype=float)
        stat, p, cohens_d = paired_wilcoxon_test(arm_ll, self_ll)
        wilcoxon[f"Affine_{arm}_vs_Self"] = {
            "statistic": stat,
            "p_value": p,
            "cohens_d": cohens_d,
            "n": len(rows),
...
def summarize() -> dict:
    """从已有 per-seed 报告构建 g0_v2_summary.json。"""
    files = sorted(glob.glob(f"{REPORT_DIR}/g0_v2_seed*.json"))
    seeds = []
    sum_ll = {}
    sum_em = {}
    sum_wilcoxon = {}
    ...
    for fp in files:
        ...
        seed = rep["seed"]
        seeds.append(seed)
        for k, v in rep["summary_ll"].items():
            sum_ll.setdefault(k, {})[str(seed)] = v
        for k, v in rep["exact_match"].items():
            sum_em.setdefault(k, {})[str(seed)] = float(v)
        for k, v in rep["wilcoxon_vs_self"].items():
            sum_wilcoxon.setdefault(k, {})[str(seed)] = v
        for h, entry in rep["summary_hop"].items():
            sum_hop.setdefault(h, {})[str(seed)] = entry
    ...
    summary = {
        ...
        "summary_ll": sum_ll,
        "exact_match": sum_em,
        "wilcoxon_vs_self": sum_wilcoxon,
        "summary_hop": sum_hop,
    }
    out = f"{REPORT_DIR}/g0_v2_summary.json"
...
def main():
    ...
    ap.add_argument("--summarize", action="store_true",
                    help="从已有 per-seed 报告构建 g0_v2_summary.json")
    a = ap.parse_args()
    if a.summarize:
        summarize()
        return
    out = a.output or f"{REPORT_DIR}/g0_v2_seed{a.seed}.json"
    ...
    print("wilcoxon_vs_self:", json.dumps(report["wilcoxon_vs_self"], indent=2))
    print("saved:", out)
